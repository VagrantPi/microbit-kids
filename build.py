...
LEARN = [
    ...
    ("radio.onNumber", "一頂<b>耳朵帽子</b>：聽到數字就做裡面的事，"
     ...
     "要<b>兩台</b>：一台按 A，另一台亮出數字（假的那台會自動變兩台）🎉"),
...
LEARN_NOPLAY = {
    ...
    "radio.setGroup": ("📡 廣播要<b>兩台</b>才看得出效果",
                       "真的板子要兩台，所以這裡沒有播放鍵。<br>不過<b>畫面上「假的那台」用到廣播就會自動變成兩台</b>，第 10 課就會玩到 👍"),
    ...
    "radio.sendString": ('📡 廣播要<b>兩台</b>才看得出效果', '真的板子要兩台，所以這裡沒有播放鍵。<br>不過<b>畫面上「假的那台」用到廣播就會自動變成兩台</b>，第 10 課就會玩到 👍'),
    "radio.onString": ('📡 廣播要<b>兩台</b>才看得出效果', '真的板子要兩台，所以這裡沒有播放鍵。<br>不過<b>畫面上「假的那台」用到廣播就會自動變成兩台</b>，第 10 課就會玩到 👍'),
}
...
# 這兩塊開新專案就在畫面上，不能叫孩子去抽屜找（他會翻不到而卡住）。
LEARN_NOFIND = {
    "basic.onStart":  "<b>不用去抽屜找</b>——開一個新專案，它<b>本來就在畫面上</b>",
    "basic.forever":  "<b>不用去抽屜找</b>——它跟「當啟動時」一樣，開新專案就在畫面上"
                      "（它是<b>基本</b>抽屜的積木，<b>藍色</b>的）",
}
...
def build_learn():
    ...
    for i, (bid, oneline, play, todo, look_txt) in enumerate(LEARN, 1):
        ...
        if play and play[0] == "sound":
            ...
            body += note(hd, txt)
        if bid in LEARN_NOFIND:
            body += '<div class="find">' + dot(b["cat"]) + LEARN_NOFIND[bid] + '</div>'
        elif findname:
    ...
    body = (
        ...
        + note("💡 旁邊開著 MakeCode 一起做",
               ...
               "一邊看這頁、一邊在那邊拖，學得最快。")
        + note("📖 這頁會用到兩個說法",
               "<b>「假的那台」</b>＝畫面左邊那台<b>假的 micro:bit</b>，"
               "不用真的板子也能先看效果（大人叫它模擬器）。<br>"
               "<b>「帽子」</b>＝上面<b>沒有缺口</b>的積木，只能放最上面，像戴一頂帽子。")
        + "".join(cards)
        + '<div class="goal win"><div class="big">🎉</div><div>'
        f'<h3>{len(LEARN)} 塊都認識了！</h3>'
        '<p>這些就是<b>接下來</b>幾課會一直用到的積木。<br>'
...
def build_index():
    ...
    body = (
        ...
        f'前面幾個<b>不用變數也不用座標</b>，做得完 👍 合計用到 {len(covered_ids())} / {len(BLOCKS)} 塊積木 🧱</span></span>'
        '<span class="go">開始 →</span></a>'
        '<h2>開始冒險 🚀</h2>'
...
def build_l2():
    body = (
        ...
        step(3, "找到藍色的「重複無限次」",
             ...
             + look("找到那塊藍色的了嗎？找到就打勾 ✅")
             + adult("這一步不動手，只是先認位置。下一步要往裡面放東西，"
                     "先確認孩子指得出那塊藍色積木，再往下走。<br>"
                     "它是<b>基本</b>抽屜的積木（藍色），不是迴圈抽屜的綠色積木——"
                     "這點常被講錯，孩子會照顏色去找。")) +
...
def build_l7():
    body = (
        ...
        step(3, "小夜燈：找到「重複無限次」",
             ...
             '<p>畫面上那塊藍色的 <b>「重複無限次」</b> 就是拿來做這個的。</p>'
             + mcp(ts_forever())
             + look("找到那塊藍色的就打勾 ✅")) +
...
def build_l11():
    body = (
        ...
        step(6, "跑到最右邊就轉頭",
             ...
             + mcp(ts_forever("basic.clearScreen()", "led.plot(x, 2)", "basic.pause(200)",
                              "x += dir", ts_if("x > 3", ("dir = -1",))))
...
GAMES = [
    ...
    dict(id="e3", em="🧭", title="歪歪指路", sub="板子歪哪邊，箭頭就指哪邊", tier="easy",
         uses=[
             ...
             # 加料關
             "basic.clearScreen", "basic.showLeds", "math.random", "logic.ifElse",
             "logic.eq",
         ]),
...
def build_games_hub():
    ...
    body = (
        ...
        '大多<b>不用盒子（變數）</b>，也<b>不用數格子（座標）</b>。<br>'
        '只有最後的<b>按按計數器</b>會用到一個盒子，想先跳過也可以 👍</p>'
        + cards_for("easy")
        ...
        + adult("入門區<b>前四個</b>的基本關<b>完全不用變數，也不用 x/y 座標</b>——"
                "這兩個概念對小一升小二是最大的門檻，所以先繞過去，"
                "讓他先嘗到「我做出一個遊戲」的成就感。<br>"
                "變數只出現在<b>加料關</b>（明講可以跳過），"
                "以及最後那個<b>按按計數器</b>——它是<b>刻意</b>拿來教變數的，"
                "等他自己說「我想記分數」再玩它，會好吸收很多。<br>"
...
def build_e3():
    body = (
        ...
        '<p><b>基本關只用兩種積木</b>，四步就能玩。</p>'
        '<p>不用盒子（變數）、不用數格子、也沒有聲音 🤫</p>'
        + tip("🔌 這個要<b>歪板子</b>才好玩",
              "做完記得<b>下載到真的板子</b>，拿在手上歪最直接。")
        ...
        step(6, "加料 ②：換成自己畫的圖",
             find("basic", "顯示指示燈")
             + '<p>把帽子裡的「顯示箭頭」換成這塊，自己點一個往左的樣子。</p>'
             + leds(LEFT, "往左：左邊兩排")
             + mc(leds_ts(LEFT))
             + look("歪左邊 → 出現<b>你畫的</b>圖案 🎨")
             + '<p class="usedhint">這一關多用到：<b>顯示指示燈</b></p>') +

        step(7, "加料 ③：先拿一塊抽籤",
             find("math", "隨機取數 0 到 10")
             + '<p>先拖到<b>空白的地方</b>放著，把 <code>10</code> 改成 <code>1</code>。</p>'
             + mc("randint(0, 1)")
             + look("積木上寫著「隨機取數 <b>0</b> 到 <b>1</b>」。")) +
        ...
        step(8, "加料 ④：搖一搖變尋寶羅盤",
             find("logic", "如果 … 那麼 … 否則", "（要有<b>「否則」</b>的那塊）")
             + find("logic", "=")
             + '<p>再拉一頂 <b>「晃動」</b> 的帽子，裡面放<b>如果…否則</b>。</p>'
             '<p>條件左邊塞<b>剛剛那塊抽籤</b>、右邊打 <code>0</code>；'
             '<b>那麼</b>放箭頭 <b>西</b>、<b>否則</b>放箭頭 <b>東</b>。</p>'
             + mcp(ts_gesture("Shake", ts_if("randint(0, 1) == 0",
                                             ("basic.showArrow(ArrowNames.West)",),
                                             ("basic.showArrow(ArrowNames.East)",))))
             + look("搖一搖 → 它<b>隨便指一邊</b>，假裝寶藏在那裡 🗺️")
             + note("🤔 搖完把板子放平，箭頭就不見了？",
                    "那是<b>加料①</b>在做事（正面朝上就清空），<b>不是壞掉</b>。<br>"
                    "想讓箭頭留著，就把「正面朝上」那頂帽子先拔掉。")
             + '<p class="usedhint">這一關多用到：<b>隨機取數</b>、<b>如果…否則</b>、<b>=</b></p>') +
...
def build_e4():
    body = (
        ...
        + step(6, "加料 ①：現在到底多亮？",
               find("basic", "顯示數字")
               + note("🔧 這一關要先把「如果」<b>拉開</b>",
                      "把整塊 <b>如果…否則</b> 拖到<b>旁邊空白處</b>放著（<b>先不要刪掉</b>），"
                      "「重複無限次」裡<b>只放</b>一塊「顯示數字」，白框框塞<b>光線感測值</b>。<br>"
                      "量完再把「如果」放回去。")
               + mcp(ts_forever("basic.showNumber(input.lightLevel())"))
               + look("數字<b>一直跑</b>——手慢慢蓋上去，<b>數字會變小</b> 🔢")
               + adult("為什麼要先把「如果」拉開：不拉開的話，蓋住時走的是「那麼」那條路、"
                       "只會看到圖案，<b>根本看不到數字</b>，門檻就量不出來。<br>"
                       "讓他記下「蓋住」和「沒蓋住」大概各是多少，中間那個數就是該填進「＜」的值。")
        ...
        step(8, "加料 ③：晚上不要太刺眼",
             ...
             + '<p>放進 <b>「當啟動時」</b> 裡面，數字改成 <code>64</code>。</p>'
             + mcp("led.setBrightness(64)")
             + note("🎩 為什麼不能放在「重複無限次」上面",
                    "「重複無限次」是<b>帽子</b>——上面<b>沒有缺口</b>，接不了東西。<br>"
                    "只要設定一次就好的事，都放進「當啟動時」。")
...
def build_e5():
    body = (
        game_top("e5", "入門遊戲 · 用一個盒子記分", "🔢 按按計數器") +
